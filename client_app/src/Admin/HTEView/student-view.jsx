...
import { Grid, List, Eye, Filter, Search, ArrowUpDown } from "lucide-react"
...
export default function StudentView({
  ...
  const filteredStudents = students.filter(student => {
    ...
    const matchesVerified = verifiedFilter === "all" ||
      (verifiedFilter === "verified" ? student.is_verified : !student.is_verified);
    const matchesApproved = approvedFilter === "all" ||
      (approvedFilter === "approved" ? student.is_approved_by_admin : !student.is_approved_by_admin);

    return matchesSearch && matchesVerified && matchesApproved;
  });
  ...
  const handleApprove = async (hteId) => {
    console.log(`Approving student ${hteId}`)
  ...
  const handleReject = async (hteId) => {
    console.log(`Rejecting student ${hteId}`)
    try {
      let res = await axios({
        method: 'put',
        url: `user/HTE/${hteId}`,
...
<div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-800">HTE Supervisor Applications</h1>
          <p className="text-gray-500">Manage and review HTE supervisor applications</p>
        </div>
      ...
      <div className="flex flex-col space-y-4">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1">
            <Input
              placeholder="Search HTE supervisors..."
              value={searchTerm}
      ...
      {view === "table" ? (
                ...
                <TableRow key={student.hteID}>
                  ...
                  <TableCell>
                    <div className="space-y-1">
                      ...
                      <p className="text-sm text-gray-500">{student.companyAddress}</p>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="space-y-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {sortedStudents.map((student) => (
            <Card key={student.hteID} className="hover:shadow-lg transition-shadow">

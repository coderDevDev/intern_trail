  ...
  const handleStatusChange = async () => {
    ...
    try {
      await axios.put(`/emergency-reports/${selectedReport.id}/status`, {
        status: newStatus,
        updatedBy: 'hte-supervisor'
      });

      // Show appropriate message based on status
      if (newStatus === 'resolved') {
        if (!selectedReport.coordinator_approval) {
          toast.info('Status updated. Waiting for OJT Coordinator approval to mark as resolved.');
        } else {
      ...
      if (error.response?.status === 403) {
        toast.error('You do not have permission to update this report');
      } else {
        ...
        <Dialog open={isViewModalOpen} onOpenChange={setIsViewModalOpen}>
          <DialogContent className="sm:max-w-[600px] max-w-[95%] mx-auto max-h-[90vh] overflow-y-auto p-4 rounded-lg">
            ...
            {selectedReport && (
              ...
              {/* Add HTE-specific approval info message */}
                ...
                <div className="bg-blue-50 text-blue-700 p-4 rounded-lg text-sm">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="mt-1">
                        As an HTE Supervisor, your approval is required along with the OJT Coordinator's
                        to mark this report as resolved. Current approvals:
                      </p>
                      <ul className="mt-2 space-y-1">
                        <li className="flex items-center gap-2">
                          ...
                          Your Approval (HTE Supervisor)
                        </li>
                        <li className="flex items-center gap-2">
                          {selectedReport.coordinator_approval ?
                            <Check className="h-4 w-4 text-green-500" /> :
                            <X className="h-4 w-4 text-gray-400" />}
                          OJT Coordinator Approval
                        </li>
                      </ul>
                    </div>
                  </div>
              ...
              <div className="flex justify-end gap-2">
                ...
                >
                  ...
                  {selectedReport.status === 'pending' ? 'Review Report' : 'Update Status'}
                </Button>
              </div>
            </div>
          )}
